...
> **Pattern**: **Framework.aDNA** (proposed reframe from the router's provisional Platform.aDNA — [[adr_000_project_identity]] D2; operator confirms at the P0 gate) — defines the **platform-agnostic git-ops standard** every `.aDNA` code-home federates against via a `git/` wrapper, the III pattern. **Persona**: **Grace Hopper** — patron of portability / machine-independence / open standards. **Status**: **`campaign_git_genesis` — "Operation Free Harbor"** — **P0–P5 ✅** — R1/P5 beachhead executed 2026-06-20 under a host-role **inversion** ([[what/decisions/adr_013_host_role_inversion|ADR-013]] supersedes ADR-005, reverses SD-1): **Git.aDNA→GitHub-public · TypeScript.aDNA→Codeberg-private**, both backends dogfooded live; predecessor archived → `Git.aDNA-legacy`. Resume-Here = **R2/P6 Fleet Alignment** (waved) — **Wave 1a + Wave 1b ✅ COMPLETE** (6 graphs → Codeberg-private: `VisualDNA`·`Lighthouse`·`Videos`·`Oration`·`Spacemacs`·`Molecules`; F1/F2 folded + dogfooded live; `Molecules` fired 2026-06-22, collision cleared). **Wave 2 (public-flips) ✅ COMPLETE 2026-06-22: `aDNA.aDNA` (canary) · `III.aDNA` · `Canvas.aDNA` → GitHub-public** (`gitops_set_visibility` first-exercised then proven across 3 graphs; all `private:false`, anon-clone succeeds, visibility-only no-shim; III's 10 leak-hits all FP → scoped allowlist; Canvas's uncommitted Salon-P0 work committed first). **`Astro` resolved** — operator **BSL-1.1 = keep private** ⇒ reclassified **W2→W3 (class I)**; GitHub-native cross-org transfer+rename → `aDNA-Network/Astro.aDNA` (stays private, no public flip; HEAD `0d0c315`; anon-clone refused). **→ Wave 2 COMPLETE. Wave 3 (internal touch-only) reachable-complete (2026-06-25): 3a 11/12 · 3b 7/8 · 3c ✅ 3/3 — only `zeta` (3a, blocked operator WIP) + `WilhelmAI` (3b, partner-org `Wilhelm-Foundation`, touch-only/keep-name) remain; `aDNALabs` reconciled (already-fired-2026-06-22 desync corrected, F-W3-e). Next = `zeta`/`WilhelmAI` when quiet → Wave 4 (nested) / Wave 5 (client) / P7.** **STATE.md is the live phase pointer — read it first.** **Thesis**: provider-agnostic abstraction (GitHub · Codeberg · self-hosted Forgejo, user's choice forever) is the *permanent architecture*; self-hosted **mesh-git is the strategic north star** (a decentralized competitor to centralized git).
