...
`how/tests/dryrun_gitops.sh` runs the lib with `GITOPS_DRY_RUN=1` for `host=github.com` **and** `host=codeberg.org` across the verbs and asserts the printed plan carries the right backend marker (`gh api` vs `/api/v1/`) — **no network, no secrets, no writes**. Live verification is the P5 beachhead dogfood.
