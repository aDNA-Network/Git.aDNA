...
created: 2026-08-19
updated: 2026-08-23   # §Constraint inputs citation repair ONLY (F-F37, Ilmarinen). Ratified D1–D5 + Amendment A1 untouched; revision stays 4 — a citation repair is not a revision.
...
**Constraint inputs (verified at source this session):** Venus's scoped §8 R&D ruling resolves placement *for the R&D window only* (revisit-at-production clause). ⚠ **Citation corrected 2026-08-23 — the original pointer resolved to nothing.** Rev 1–4 cited `Network.aDNA/who/governance/ruling_2026_08_07_scoped_s8_rd_forge.md`; **that file has never existed**, at that path or any other (`who/governance/` holds three `ruling_*` files and none is this one — re-verified at the object here, not transcribed). The path originated in `Forgejo.aDNA`'s `adr_001_rd_node_placement` on 2026-08-07 and this lane picked it up in good faith; Ilmarinen found it and routed it rather than fixing only their own copy, because *"verified at source this session"* is a certification only this lane can make or withdraw (**F-F37**, theirs). **The ruling itself is entirely real and its content is unchanged** — this is a pointer defect, not a doctrine defect. Two sources that resolve: the operator Shape-A scoped-R&D ruling at `Network.aDNA/how/campaigns/campaign_harbor/artifacts/brief_jake_l1_rd_lighthouse_shapes.md` §110, and Venus accepting it as her own in `coord_2026_08_07_venus_to_ilmarinen_shape_a_reconciliation_and_ack` §1 (*"accepted as-is, no amendment"*). ⛔ **Recorded rather than silently repointed**: an auditor who follows a dead path cannot tell a broken pointer from a fabricated authority, and overwriting it quietly removes the only trace that the check was ever owed. ⚖ How it survived fourteen days, which is the transferable half: the ruling's *content* was restated in full in both documents, so nobody ever needed to follow the pointer — **a reference that is always restated is a reference that is never resolved**, the same shape as the full-history secret gate that only a check exercises. ⚠ **The ruling is scoped and expiring by intent** — *"production placement is a NEW decision, not an inheritance"* — which is the D4 revisit's actual premise, not a note about it. Forgejo OAuth2 has **no scope enforcement** (Forgejo.aDNA D12 §D.6). The Exchange's anonymous fetcher sets `follow_redirects=False` and consumes the raw-fetch shape `branch/<branch>/<path>`. Exchange ADR-041 (ratified 2026-08-15) fixed `Git.aDNA`'s manifest lane: **public → `aDNA-Commons`**.
