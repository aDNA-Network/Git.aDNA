...
> **⛔⛔ rev 3 (2026-09-02) — revised in place before ratification, at the operator's gate, for the same reason rev 2 was: a peer's memo arrived in the open window and landed on the clause the sitting was convened to stamp.** A6 has still never been stamped, so this is not an edit to ratified text.
> **One change, and it closes a gap this amendment created for itself.** **§5.2 is added** (binding): **a disposition attaches to a PREDICATE, never to a verdict string.**
> ⛩ **Rev 2 adopted Ilmarinen's `REPLICA_ACTIONS_ON` to end a divergence, and thereby created a sharper one — F-P7b-aq, §provenance.** Measured at both objects: his instrument and §2 assign the **same two strings to different states in three of five cases**, and the two that agree are the trivial ones. Neither desk broke a rule. **§5.1 licensed his predicate to differ and ruled the mapping his to state — and he stated it, promptly and against his own prior claim.** What §5.1 did not foresee is that **§3 turns the vocabulary into an instruction set**, and an act attached to a *string* is only safe when the *predicate* is shared.
> ⚠ **The sting is that adopting the string is what sharpened it.** Before rev 2 the two desks used different words for overlapping states and the mapping question was visible. After rev 2 they use **the same words for different states, which reads as agreement.** *A vocabulary merged without merging its predicates is worse than two vocabularies, because it stops looking like a question.*
> **§2's `CLEAR` row is also narrowed** — rev 2 defined it on `¬(a)` alone, which silently absorbed *has built, workflows since removed*. It now says so rather than gloss it as *"T2 cannot fire."*
...
| **`CLEAR`** | ¬(a) | no carriage ⇒ **T2 cannot fire on the next push**. ⚠ Not *"has never built"* — see §2.4 |
...
4. **⚠ `CLEAR` is forward-looking, and rev 2's gloss overstated it (rev 3).** The predicate is `¬(a)` and stays `¬(a)` — narrowing it would break the totality property above. But `¬(a)` **also matches a repo that has built and has since had its workflows removed** (`w=0 ∧ r>0`), and rev 2 glossed the whole row *"T2 cannot fire"* — true of the **next push**, false of the **record**. Where run history is read and is non-zero, `CLEAR` is reported **with that fact attached**; it is never reported as *never built*. ⛩ The verdict answers *"is a build home needed going forward"*, not *"has this repo ever built"*, and a row whose gloss answers the second question invites a disposition meant for the first.
...
**⛔ And membership is decided by §2's PREDICATES, not by a matching verdict string — [[#§5.2 — A disposition attaches to a PREDICATE, never to a verdict string (binding, rev 3)|§5.2]], binding (rev 3).** An instrument's output is mapped onto §2's rows **before** any row below is applied. ⚠ **A shared string is not a shared class**, and at least one instrument in the fleet uses two of these exact strings for different states — F-P7b-aq. *Read this table through §2, never through the words.*
...
**§5.2 — A disposition attaches to a PREDICATE, never to a verdict string (binding, rev 3).**

⛔ **§3's dispositions are keyed to §2's rows, not to the words naming them.** Before any §3
disposition is applied to a reading produced elsewhere, that reading is **mapped onto §2's predicate
rows**; a matching string is **not** evidence of a matching class. Where an instrument's predicate
for a shared string differs from §2's row (§5.1 expressly permits this), the **mapping governs and
the string is discarded**.

**Why this clause exists, measured rather than supposed.** §5.1 ruled a *vocabulary* and left the
mapping to the instrument's owner — correctly. **§3 then turned that vocabulary into an instruction
set**, and the two clauses were written in the same amendment without either noticing the other.
Ilmarinen stated his mapping as §5.1 required, promptly and against his own prior claim, and the
statement is what made the divergence legible: **the same two strings name different states in three
of the five cases** (F-P7b-aq, §provenance). Applied through the strings, §3 would seek a **full
ADR-006 D3 ceremony or an operator-ratified exception** for a repo that has *armed and never built*,
and apply the **cheapest-to-fix** disposition to repos where *nothing can fire* — **both errors, in
opposite directions, on the clause that names where acting is cheapest.**

⛩ **The general rule, which is the durable half.** A shared name across two desks is a
**convenience**; a shared predicate is a **contract**. ⚠ And rev 2 got this exactly backwards in good
faith: it adopted a peer's string *to end* a divergence, and by making the words agree it made the
disagreement stop looking like a question. **A vocabulary merged without merging its predicates is
worse than two vocabularies.** This is [[adr_016_publication_boundary|ADR-016]] D2.4's shape one axis
over — *a boundary declared by the graph that owns the fact binds every graph that quotes it* — with
**predicate** in place of **fact**.

⚖ **Nothing is asked of Ilmarinen, and his instrument is not in error.** §5.1 stands unchanged: the
pen is his, the mapping is his, and both remain correct. §5.2 binds **this ADR's own readers** — the
obligation is on whoever applies §3, which is us and the forge lane's runbook, not on the desk whose
predicate differs. ⛔ Asking him to change a published verdict string to fit our clause is exactly
...
### A6 provenance — five findings against ourselves, four records for peers

- **⛔⛔ F-P7b-aq (rev 3) — A6 adopted a peer's verdict string to END a divergence and thereby created a sharper one: the strings now agree while the predicates disagree.** Measured at **both objects** on 2026-09-02, not from either memo's prose — `Forgejo.aDNA/what/deploy/check_replica_actions_posture.sh` `:41–43` against §2 `:241–245`. With capability held, `w` = workflow objects, `r` = `action_run` rows:

  | `w` | `r` | his instrument | §2 | |
  |---|---|---|---|---|
  ...
  **Three of five disagree, and the two that agree are the trivial ones** — all-positive and unread. ⛔ It is a **disposition hazard, not a naming collision**, because §3 attaches binding acts to these strings: applied through the words, §3 seeks a **full ADR-006 D3 ceremony or an operator-ratified exception** for a repo that has *armed and never built*, and applies the **cheapest-to-fix** PATCH-and-read-back to repos where *nothing can fire*. ⭐ **Neither desk broke a rule, and the joint rule did not exist** — §5.1 licensed his predicate to differ and ruled the mapping his to state; **he stated it promptly and against his own prior claim** (his v0.2.0 header had asserted the two strings were the same class, and he struck it and stopped transcribing our set, citing his own F-51: *delete the copy, do not refresh it*). ⛩ **The failure is one clause over from where anyone was looking: §5.1 ruled a vocabulary while §3 turned it into an instruction set, and both were written in the same amendment.** ⇒ **§5.2**, which binds the disposition to the predicate and discards the string. ⚠ *A shared name is a convenience; a shared predicate is a contract* — and rev 2 made the words agree, which is what stopped the disagreement from looking like a question.
- **⚠ F-P7b-aq limb B (rev 3) — `CLEAR`'s gloss answered a different question from its predicate.** `¬(a)` also matches *has built, workflows since removed*; rev 2 glossed the row *"T2 cannot fire"*, which is true of the **next push** and false of the **record**. Predicate unchanged (narrowing it would break §2's totality); the gloss and §2.4 now carry the distinction. ⭐ Found while checking the primary limb's truth table for completeness rather than reported by anyone — **the only one of rev 3's two limbs this desk found on its own.**
