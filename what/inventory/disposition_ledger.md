...
| `Molecules.aDNA` | GH-priv `aDNA-Network/moleculeforge` | **P-dev** ✅ | Codeberg-private `Molecules.aDNA` | ⚠️→✅ FP-only (2026-06-21) | ✅ | **HELD (turnkey, separate gate)** — **name-drift** → rename-on-host (Berthier); 16 gitleaks hits = **confirmed false-positives** (8-char verdict enums `accepted`/`rejected` + module-type `verifier` in `execution_result.json`; `generic-api-key` on dict-words) → **NO secret, NO rotation**; scoped per-graph `git/.gitleaks.toml` allowlist staged (hook reads it directly post-F2) |
| `Oration.aDNA` | GH-priv `aDNA-Network/SpeechForge.aDNA` | **P-dev** ✅ | Codeberg-private `Oration.aDNA` | ✅ clean (2026-06-21) | ✅ | **HELD (turnkey, separate gate)** — **name-drift** → rename-on-host (Berthier); genesis P1 |
| `Spacemacs.aDNA` | **LatticeProtocol**/`Spacemacs.aDNA` | **P-dev** · **FOSS-intent CONFIRMED 2026-06-21** | Codeberg-private | ✅ clean (2026-06-21) | ✅ | **HELD (turnkey, separate gate)** — **cross-org** → migrate `aDNA-Network` (Berthier); v1.0.0; FOSS-intent confirmed (operator; GPLv3-fork lineage) → Codeberg; the "if internal → W3" branch retired |
