...
| `Molecules.aDNA` | GH-priv `aDNA-Network/moleculeforge` | **P-dev** ✅ | Codeberg-private `Molecules.aDNA` | ⚠️→✅ FP-only (2026-06-21) | ✅ | **HELD — COLLISION (2026-06-21)** — pre-flight found an **active `session_sc_m4` + dirty tree** (sc_web_configurator work); host-move deferred until clean (turnkey otherwise — runbook current). **name-drift** → rename-on-host (Berthier); 16 gitleaks hits = **confirmed false-positives** (8-char verdict enums `accepted`/`rejected` + module-type `verifier` in `execution_result.json`; `generic-api-key` on dict-words) → **NO secret, NO rotation**; scoped per-graph `git/.gitleaks.toml` allowlist staged (hook reads it directly post-F2) |
