...
### `Molecules.aDNA` (class P-dev · branch `main` · **name-drift** `MoleculeForge.aDNA`→`Molecules.aDNA`) — ✅ EXECUTED 2026-06-22
> **As-fired (2026-06-22):** the prior collision (`session_sc_m4` + dirty `sc_web_configurator` tree) **cleared** — `session_sc_m4` completed (vault `246f8f9` + code `98b88f7`/`v0.14.0`), tree clean. `origin` = `codeberg.org/aDNA-Network/Molecules.aDNA` (private; HEAD `be9c4b9`; old `MoleculeForge.aDNA` → remote `rollback` 30d). Branch `main` ⇒ **F1** = `main→main` no-op (like the Videos canary). **F2** pre-push hook dogfooded clean **resolving the per-graph `git/.gitleaks.toml` allowlist** (16 `execution_result.json` verdict-enum FPs) — the allowlist's first live exercise. Verified: `private:true`, anon-refused, authed-clone HEAD==`origin/main`. Wrapper+doctrine+hook+STATE/MANIFEST applied. **→ Wave 1b COMPLETE (4/4).** The as-staged recipe below is retained as the as-built record.
