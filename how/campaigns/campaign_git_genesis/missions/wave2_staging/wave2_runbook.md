...
> **STATUS: Wave 2 — 3/4 EXECUTED 2026-06-22.** `aDNA.aDNA` (canary) + **`III.aDNA` ✅** + **`Canvas.aDNA` ✅** → GitHub-public (all `private:false`, anon-clone succeeds, visibility-only no-shim; `gitops_set_visibility` proven across 3 graphs). **`Astro.aDNA` HELD** — cross-org + name-drift + **BSL-1.1 posture flag** → Berthier (coord drafted). Each public-flip is **outward + irreversible-exposure** → fired only at its operator gate (rollout DP5), per-graph, with a **mandatory full-history secret-scan**.
...
### `Astro.aDNA` (class P-released · branch `main` · **cross-org + name-drift**) — HELD (coord drafted 2026-06-22)
- **Verified 2026-06-22:** origin `LatticeProtocol/SiteForge.aDNA` is **private**, **BSL-1.1** licensed, status `active`, `main` ahead 5; target `aDNA-Network/Astro.aDNA` absent (404). **Sequence:** Berthier coord → transfer `LatticeProtocol/SiteForge.aDNA` → `aDNA-Network` + rename → `Astro.aDNA` → **BSL-1.1 posture decision** (public-source-available vs keep-private/reclassify) → fresh full-history scan → public-flip (or reclassify). **NEW FLAG:** BSL-1.1 is source-available, not classic FOSS — surfaced for an explicit posture call before exposure. Coord: [[coord_2026_06_22_git_wave2_astro_cross_org]]. Host-move ⇒ **gets a §C shim** (unlike III/Canvas).
