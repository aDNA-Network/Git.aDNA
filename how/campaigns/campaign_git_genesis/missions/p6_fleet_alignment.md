...
2. **Wave 1 — low-risk internal** — vaults with no LFS / client data / live consumers → Codeberg. **🔵 Wave 1a ✅ FIRED 2026-06-21** (`VisualDNA` + `Lighthouse` → Codeberg-private greenfield first-remotes). **🔵 Wave 1b CANARY (`Videos`) ✅ FIRED 2026-06-21** (first host-move; Wave-1a F1/F2 folded + dogfooded live). **Held (turnkey):** `Molecules`·`Oration`·`Spacemacs` (host-moves) + **Wave 2** (public flips) remain separately gated.
