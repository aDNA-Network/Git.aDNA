...
## Exit Gate
An **integration ADR** (co-authored/ratified with Network.aDNA) fixing forge-placement, the context-sync protocol, the identity bridge, and addressing; the Network coord memo updated + delivered; inputs ready for P7b + the `Lighthouse.aDNA` deployable. **Status: ADR-015 at rev 3 (D1.5's egress restore corrected against the Exchange's guard implementation 2026-08-19); gate = Venus concurrence + operator §7.7, both against rev 3 (pending).**
